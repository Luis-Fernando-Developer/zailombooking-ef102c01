...
export function PaymentSettings({ companyId }: Props) {
  ...
  async function load() {
    ...
    if (s) {
      setSettings({
        ...
        own_gateway_provider:
          (s.own_gateway_provider as Provider) || "asaas",
        payout_flow:
          (s.payout_flow as any) || "via_company",
      });
  ...
  async function validateKey() {
    ...
    try {
      const { data, error } = await supabase.functions.invoke(
        "validate-own-gateway-key",
        {
          body: {
            api_key: apiKeyInput.trim(),
            provider: settings.own_gateway_provider,
          },
        }
      );
      ...
      setValidatedAccount(
        (data as any).account_name || "Conta validada"
      );

      toast({
        title: "Chave válida",
        description: (data as any).account_name,
      });
    } catch (e: any) {
      toast({
  ...
  function changeProvider(p: Provider) {
    ...
    Object.keys(METHOD_META).forEach((k) => {
      newMethods[k] = supported.includes(k)
        ? !!settings.accepted_methods?.[k]
        : false;
    });

    setSettings({
      ...settings,
      own_gateway_provider: p,
      accepted_methods: newMethods,
    });
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Decida se seus clientes podem pagar online ao agendar. Quando
            ativado, você usa sua própria conta de gateway — o pagamento
            cai 100% na sua conta, sem taxas adicionais da plataforma.
          </CardDescription>
        </CardHeader>
      ...
      {enabled && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Escolha o provedor e conecte sua conta.
            </CardDescription>
          </CardHeader>
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-2">
                {(Object.keys(PROVIDER_INFO) as Provider[]).map(
                  (p) => (
                    <button
                      key={p}
                      type="button"
            ...
            <div className="rounded-lg border bg-muted/40 p-4 space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Cadastre esta URL no painel do {info.label} para que
                confirmações de pagamento atualizem o status do
                agendamento automaticamente. {info.webhookHelp}
              </p>
      ...
      {enabled && (
        <Card className="relative overflow-hidden">
          <div className="pointer-events-none absolute inset-0 z-20 bg-background/55 backdrop-blur-[1px]" />

          <div className="pointer-events-none absolute inset-0 z-30 flex items-center justify-center p-6">
            <div className="rounded-xl border bg-background/95 shadow-lg px-6 py-5 text-center max-w-sm">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                O fluxo de repasse para autônomos está sendo
                desenvolvido e estará disponível em breve.
              </p>
            </div>
          </div>
          ...
          <CardHeader>
            <CardTitle>
              Fluxo de repasse para autônomos
            </CardTitle>

            <CardDescription>
              Define o caminho padrão do dinheiro quando um cliente
              paga um agendamento de um profissional autônomo.
              Você pode sobrescrever caso a caso na ficha de cada
              autônomo (Equipe → Editar).
            </CardDescription>
          </CardHeader>
          ...
          <CardContent className="space-y-3">
            {[
              ...
              <button
                ...
                type="button"
                disabled
                className="w-full p-4 border rounded-lg text-left opacity-60 cursor-not-allowed"
              >
                <div className="font-medium text-sm">
                  {o.title}
                </div>
      ...
      {enabled && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Habilite as formas de pagamento. Métodos não suportados
              pelo {info.label} ficam desabilitados.
            </CardDescription>
          </CardHeader>
