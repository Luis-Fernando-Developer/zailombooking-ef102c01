...
export function BusinessLayout({ children, companySlug, companyName, companyId, userRole, currentUser, hideHeader = false }: BusinessLayoutProps) {
  const { toast } = useToast();
  useEffect(() => { applyTheme(getInitialTheme("admin")); }, []);
  ...
  return (
    <div className="flex w-full min-h-screen">
      ...
      <BusinessSidebar companySlug={companySlug} companyName={companyName} companyId={companyId} userRole={userRole} currentUser={currentUser} />

      <div className="flex flex-col flex-1 min-h-screen transition-[margin,width] duration-700 ease-[cubic-bezier(0.4,0,0.2,1)] relative overflow-hidden">
        {!hideHeader ? (
          <header className="min-h-[70px] shrink-0 w-full flex items-center border-b border-primary/20 bg-card/80 backdrop-blur-2xl px-3 sm:px-4 z-40 gap-2">
            <div className="ml-1 sm:ml-4 flex flex-col py-2 sm:py-3 min-w-0 flex-1">
              ...
              <div className="mt-0.5 flex items-center gap-1 text-[11px] sm:text-sm text-muted-foreground min-w-0">
                ...
                <button type="button" onClick={handleCopy} aria-label="Copiar link público" className="shrink-0 rounded p-1 hover:bg-primary/10 text-primary">{copied ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}</button>
              </div>
            </div>
            <div className="ml-auto flex items-center gap-1 shrink-0">
              <ThemeToggle /><NotificationsBell companyId={companyId} companySlug={companySlug} />
            </div>
          </header>
        ) : (
        ...
        <main className={`flex-1 ${hideHeader ? 'overflow-hidden h-screen' : 'overflow-y-auto'} bg-gradient-hero scrollbar-thin scrollbar-thumb-primary/20 scrollbar-track-transparent`}>
          <div className={`w-full ${hideHeader ? 'h-full flex flex-col' : ''}`}>
            {!hideHeader && (<div className="px-4 pt-4"><SubscriptionSuspendedBanner companyId={companyId} /><PlanOverageBanner companyId={companyId} /></div>)}
            <div className={hideHeader ? "flex-1 overflow-hidden" : ""}>{children}</div>
          </div>
        </main>
      </div>
    </div>
