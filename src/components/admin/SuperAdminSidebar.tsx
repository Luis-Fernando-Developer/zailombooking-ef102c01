...
import { LayoutDashboard, Building2, Layers, CreditCard, Settings, LogOut, Sparkles, Megaphone, Wallet } from "lucide-react";
...
export function SuperAdminSidebar() {
  ...
  const handleLogout = async () => {
    try { await supabase.auth.signOut(); toast({ title: "Logout realizado", description: "Até logo!" }); navigate('/super-admin/login'); } catch {}
  };

  const isActive = (path: string) => location.pathname === path;
  const getNavCls = (active: boolean) => active ? "bg-primary/20 text-primary border-r-2 border-primary" : "hover:bg-primary/10 hover:text-primary";
      ...
      <SidebarContent className="bg-card/30 backdrop-blur-sm border-r border-primary/20 min-h-0 overflow-hidden">
        <div className="p-4 border-b border-primary/20 flex flex-col items-center justify-center min-h-[100px]">
          {!collapsed ? (
            ...
            <div className="flex justify-center"><BookingLogo showText={false} /></div>
          )}
        </div>
                ...
                {menuItems.map((item) => (
                  <SidebarMenuItem key={item.title}>
                    <SidebarMenuButton asChild>
                      <NavLink to={item.url} className={cn("flex items-center gap-3 px-3 py-2 rounded-lg transition-all duration-300", collapsed ? "justify-center w-10 h-10 p-0" : "w-full", getNavCls(isActive(item.url)))}>
        ...
        <div className="mt-auto p-4 border-t border-primary/20">
          <Button variant="ghost" size="sm" onClick={handleLogout} className={cn("flex items-center gap-3 px-3 transition-all duration-300", collapsed ? "justify-center w-10 h-10 p-0" : "w-full justify-start", "hover:bg-destructive/10 hover:text-destructive")}>
            <LogOut className={`w-5 h-5`} />{!collapsed && <span>Sair</span>}
          </Button>
        </div>
      </SidebarContent>
    </Sidebar>
