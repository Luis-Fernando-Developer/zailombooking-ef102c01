...
export default function BusinessFinance() {
  ...
  async function load() {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .select("id,booking_id,status,method,amount,external_id,created_at,paid_at,booking:bookings(id,booking_date,client:clients(name),service:services(name))")
        .eq("company_id", c.id)
  ...
  const totals = useMemo(() => {
    ...
    return { total: filtered.reduce((s, p) => s + Number(p.amount || 0), 0), paid: paid.reduce((s, p) => s + Number(p.amount || 0), 0), pending: pending.reduce((s, p) => s + Number(p.amount || 0), 0), cancelled: cancelled.reduce((s, p) => s + Number(p.amount || 0), 0), count: filtered.length };
  }, [filtered]);
