...
export default function BusinessServices() {
  ...
  if (visibleTabs === 0) {
    return (
      <BusinessLayout companySlug={company.slug} companyName={company.name} companyId={company.id} userRole={employee.role}>
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            <h2 className="text-2xl font-bold text-destructive">Acesso Negado</h2>
            <p className="text-muted-foreground">Você não possui permissão para acessar as abas de Serviços.</p>
          </div>
        </div>
      </BusinessLayout>
    );
  ...
  const defaultTab = canSeeServices ? 'services' : canSeeCombos ? 'combos' : 'rewards';
    ...
    >
      <div className="p-6">
        ...
        <Tabs defaultValue={defaultTab} className="mb-6">
          <TabsList className="grid" style={{ gridTemplateColumns: `repeat(${visibleTabs}, minmax(0, 1fr))` }}>
            {canSeeServices && <TabsTrigger value="services" className="flex items-center gap-2"><Briefcase className="w-4 h-4" /> Serviços</TabsTrigger>}
            {canSeeCombos && <TabsTrigger value="combos" className="flex items-center gap-2"><Package className="w-4 h-4" /> Combos</TabsTrigger>}
            {canSeeGifts && <TabsTrigger value="rewards" className="flex items-center gap-2"><Gift className="w-4 h-4" /> Brindes</TabsTrigger>}
          </TabsList>
                    ...
                    <Card key={combo.id} className="relative">
                      ...
                      <CardContent>
                        <div className="space-y-2 text-sm">
                          <div className="flex justify-between"><span className="text-muted-foreground">Preço do combo:</span><span className="font-semibold">{formatPrice(combo.price)}</span></div>
                          <div className="flex justify-between"><span className="text-muted-foreground">Preço original:</span><span className="line-through text-muted-foreground">{formatPrice(combo.original_total_price || 0)}</span></div>
                          <div className="flex justify-between"><span className="text-muted-foreground">Duração:</span><span>{formatDuration(combo.total_duration_minutes || 0)}</span></div>
                          <div className="mt-3 pt-3 border-t">
                            <p className="text-xs text-muted-foreground mb-2">Serviços inclusos:</p>
                            {combo.items?.map((it) => <div key={it.service_id} className="text-sm">• {it.service?.name || 'Serviço não encontrado'}</div>)}
                          </div>
                        </div>
                      </CardContent>
                    </Card>
