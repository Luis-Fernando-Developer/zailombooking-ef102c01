...
export default function HumanResources() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const resolvedRole = employee?.role || (companyData.owner_id === authData.user.id ? "owner" : "employee");
  ...
  const { hasPermission, loading: permissionLoading } = usePermissions(company?.id, currentUser);
  const isAdministrator = userRole === "owner" || userRole === "admin";
      ...
      <div className="p-4 sm:p-6 sm:px-10 w-full space-y-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Gestão administrativa e acompanhamento da equipe da empresa.
          </p>
        </div>
          ...
          <Card>
            <CardContent className="p-5 flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold mt-1">{inactiveEmployees}</p>
              </div>
          ...
          <Card>
            <CardContent className="p-5 flex items-center justify-between">
              <div>
                ...
                <p className="text-sm font-semibold mt-2">Estrutura inicial</p>
              </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              O RH fica separado da gestão operacional de Colaboradores. Cada módulo terá sua própria permissão.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {hrModules
                .filter((module) => isAdministrator || hasPermission(module.permission) || hasPermission("hr.view"))
                .map((module) => {
                  const Icon = module.icon;
                  return (
                    <Card key={module.title} className="bg-card/60">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Visão rápida dos colaboradores cadastrados na empresa.</CardDescription>
          </CardHeader>
          <CardContent>
                  ...
                  <div key={employee.id}>
                    <div className="flex items-center justify-between gap-4 py-2">
                      <div className="min-w-0">
                        ...
                        <p className="text-sm text-muted-foreground">{roleLabel(employee.role)}</p>
                      </div>
                      <Badge variant={employee.is_active === false ? "secondary" : "outline"}>
                        {employee.is_active === false ? "Inativo" : "Ativo"}
                      </Badge>
                    </div>
