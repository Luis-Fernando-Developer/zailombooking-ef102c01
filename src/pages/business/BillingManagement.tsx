...
export default function BillingManagement() {
  ...
  // (empresas criadas pelo super admin).
  ...
  async function fetchAll() {
    ...
    try {
      ...
      /*
       * Limpa invoices órfãs de proração antes de carregar
       * o histórico financeiro.
       *
       * Essa limpeza não deve impedir a tela de cobrança
       * de carregar caso a RPC ainda não tenha sido criada
       * ou ocorra qualquer outro erro.
       */
  ...
  /**
   * Define quais status permitem que uma invoice seja paga.
   *
   * IMPORTANTE:
   * - paid       -> já paga, não pode gerar/pagar novamente
   * - cancelled  -> cancelada, não pode pagar
   * - refunded   -> estornada, não pode pagar
   * - pending    -> pode pagar
   * - overdue    -> pode pagar
   * - processing  -> ainda pode existir fluxo de pagamento
   */
  ...
  async function handleGenerateCharge(
    ...
    try {
      ...
      if (billingType === "PIX" && result?.pix_payload) {
        ...
        // O webhook global cuidará da atualização automática do status.
      } else if (result?.invoice_url) {
        window.open(
          result.invoice_url,
          "_blank",
            ...
            <Card>
              <CardHeader>
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <CardDescription>
                      {subscription ? (
                        <div className="flex flex-col">
                          <span>
                            {formatBRL(subscription.original_price)} /{" "}
                  ...
                  <Badge
                    variant={
                      effectiveSubStatus(subscription) === "active"
                        ? "default"
                        : "destructive"
                    }
                    className="self-start"
          ...
          <TabsContent value="methods" className="space-y-4">
            <Card>
              ...
              <CardContent className="space-y-3">
                {methods.length === 0 && (
                  <p className="text-sm text-muted-foreground">
                    Nenhum método cadastrado. Adicione um cartão
                    ou use PIX.
                  </p>
                )}
              ...
              <CardContent>
                <Table>
                  ...
                  <TableBody>
                    ...
                    {invoices
                      // Filtra duplicatas pendentes do mesmo
                      // vencimento caso já exista uma paga.
                      .filter((inv, idx, self) => {
                        if (inv.status !== "pending") {
                          return true;
                        }

                        const hasPaidSameDay = self.some(
                          (s) =>
                            s.status === "paid" &&
                      ...
                      .map((i) => (
                        <TableRow key={i.id}>
                          ...
                          <TableCell className="text-right">
                            <div className="flex flex-wrap justify-end gap-2">
                              {/* PIX:
                                  somente invoices pagáveis */}
                              {isInvoicePayable(i.status) &&
                                i.pix_payload && (
                                  <Button
                                    size="sm"
                                    onClick={() =>
        ...
        <DialogContent className="sm:max-w-[400px]">
          ...
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              ...
              responsável para emitir a cobrança. Ele fica salvo
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(val) => {
                  ...
                  // Reset period when changing plan to avoid confusion
                  // if needed, but usually better to keep what user selected.
                }}
              >
                <SelectTrigger className="w-full">
                  <SelectValue placeholder="Selecione um plano" />
 ...
 * (suspended/blocked/paused/past_due).
 *
 * `status` é apenas o estado do contrato, por isso pode
 * continuar "active" mesmo com fatura em aberto.
 *
 * Aqui priorizamos o billing_status quando ele não estiver ativo.
 */
