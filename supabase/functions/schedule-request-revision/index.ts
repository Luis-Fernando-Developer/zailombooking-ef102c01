// Deploy: supabase functions deploy schedule-request-revision --no-verify-jwt
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    await admin.from('schedule_audit_log').insert({
      schedule_id, tenant_id, action: 'revision_requested', actor_user_id: user.id, reason,
    });

    if (schedule.created_by) {
      await admin.from('company_notifications').insert({
        company_id: tenant_id, type: 'schedule_revision',
        title: 'Revisão solicitada na escala',
    ...
    console.error('schedule-request-revision', e);
