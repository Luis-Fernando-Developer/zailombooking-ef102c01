// Deploy: supabase functions deploy schedule-approve --no-verify-jwt
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!tenant_id || !schedule_id) return j({ error: 'tenant_id, schedule_id obrigatórios' }, 400);
    ...
    if (schedule.status !== 'pending_approval') return j({ error: 'invalid_status', status: schedule.status }, 400);
    ...
    await admin.from('schedule_audit_log').insert({
      schedule_id, tenant_id, action: 'approved', actor_user_id: user.id, reason: reason ?? null,
    });

    // notifica o criador
    if (schedule.created_by) {
      await admin.from('company_notifications').insert({
        company_id: tenant_id, type: 'schedule_approved',
        title: 'Escala aprovada',
        message: `${schedule.name} foi aprovada.`,
