-- ============================================================================
-- 2067 — Permitir que o painel administrativo ignore apenas a antecedência mínima
--
-- Contexto:
--   A função public.get_available_slots é usada tanto pelo agendamento público
--   quanto pelo painel da empresa. Para o público, a antecedência mínima deve
--   continuar sendo respeitada. No painel administrativo, porém, o dono/gestor
--   pode abrir/estender o horário no mesmo dia e criar um encaixe manual para
--   os próximos minutos.
--
-- Correção:
--   1) Adiciona p_ignore_min_advance BOOLEAN DEFAULT FALSE.
--   2) Continua bloqueando horários que já passaram no dia atual.
--   3) Ignora somente a janela extra de antecedência mínima quando o painel
--      chamar a RPC com p_ignore_min_advance = TRUE.
-- ============================================================================
...
BEGIN
  ...
  WHILE v_cur + (v_duration||' min')::INTERVAL <= v_end::INTERVAL LOOP
    ...
    -- Para o público, aplica antecedência mínima. Para o painel administrativo,
    -- p_ignore_min_advance = TRUE libera encaixes próximos, mas não no passado.
    IF p_date = v_today
...
-- Mantém compatibilidade explícita com chamadas antigas de 4 parâmetros quando
-- o cache de schema ainda resolve a assinatura anterior.
