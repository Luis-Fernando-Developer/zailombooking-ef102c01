-- ============================================================================
-- 2057 — Hotfix da 2056: coluna correta é business_hours.is_open (não is_closed).
--
-- Sintoma (após 2056):
--   GET /v1/availability/dates e /slots → 500
--   { "error": "column \"is_closed\" does not exist" }
--
-- Correção:
--   Reescreve public.get_available_slots reaproveitando a lógica comprovada
--   da migração 2034 (DOW 0..6, entry_type IN ('F','A','FE','D','DO'),
--   COALESCE(bh.is_open, TRUE) = TRUE) e preserva a correção de fuso
--   introduzida na 2056 para a checagem de conflito com bookings
--   (usa booking_time local; fallback via AT TIME ZONE 'America/Sao_Paulo').
-- ============================================================================
...
BEGIN
  ...
  WHILE v_cur + (v_duration||' min')::INTERVAL <= v_end::INTERVAL LOOP
    ...
    -- Usa booking_time (TIME local — populado pelo trigger da 2054) quando
    -- disponível; caso contrário converte start_time/end_time para
    -- America/Sao_Paulo. NUNCA usa ::TIME direto no timestamptz — a sessão
    -- vem em UTC e o slot local (14:00) nunca casaria com 17:00 UTC.
    IF EXISTS (
