-- ============================================================================
-- 2174 - Corrige link da notificação de brinde para o slug da empresa
-- ============================================================================
...
BEGIN
  ...
  SELECT user_id INTO v_user_id FROM public.clients WHERE id = v_booking.client_id AND company_id = v_booking.company_id;
  SELECT slug INTO v_company_slug FROM public.companies WHERE id = v_booking.company_id;
  ...
  LOOP
    ...
    WHILE v_existing < v_to_award LOOP
      ...
      INTO v_service_ids
      FROM public.client_reward_services crs WHERE crs.reward_id = v_reward.id;
      ...
      IF FOUND THEN
        ...
        IF v_user_id IS NOT NULL THEN
          ...
          WHERE client_id = v_booking.client_id AND reward_id = v_reward.id AND achievement_number = v_next;

          SELECT string_agg(s.name, ', ' ORDER BY s.name)
          INTO v_service_names
          FROM public.services s WHERE s.id = ANY(COALESCE(v_service_ids, '{}'::UUID[]));

          INSERT INTO public.company_notifications (
            company_id, target_user_id, type, title, message, link, metadata
          )
          VALUES (
            v_booking.company_id, v_user_id, 'reward_earned',
            '🎁 Você ganhou um brinde!',
            format(
              'Parabéns! Você atingiu %s procedimentos e ganhou "%s". %s%s',
              v_reward.required_procedures, v_reward.name,
              CASE WHEN COALESCE(v_service_names, '') <> '' THEN 'Você pode resgatar: ' || v_service_names || '. ' ELSE '' END,
              CASE WHEN COALESCE(v_reward.reward_value, 0) = 0 THEN 'Valor: grátis.'
                   ELSE format('Valor especial: R$ %s.', to_char(v_reward.reward_value, 'FM999999990D00')) END
            ),
            CASE WHEN COALESCE(v_company_slug, '') <> '' THEN '/' || v_company_slug || '/client/premios' ELSE '/client/premios' END,
            jsonb_build_object(
              'achievement_id', v_achievement_id, 'reward_id', v_reward.id,
              'expires_at', v_expiry, 'source', 'completed_booking'
            )
          );
        END IF;
      END IF;
