-- 2184 — Pagamento local é considerado pago quando o serviço é concluído
BEGIN;
...
BEGIN
  IF LOWER(COALESCE(NEW.booking_status::text, '')) = 'completed'
     ...
     AND LOWER(COALESCE(NEW.payment_status::text, '')) IN ('pending', 'confirmed') THEN
...
WHERE LOWER(COALESCE(booking_status::text, '')) = 'completed'
  ...
  AND LOWER(COALESCE(payment_status::text, '')) IN ('pending', 'confirmed');
